...
def sentence_split_1(text, question, answer):
    first_and_text = ''
...
def sentence_split(text, question, answer):
    ...
    for k, j in enumerate(wordlist):
        if answer in j or j in answer:
            if k != 0 and k != 1 and k != len(wordlist) - 1 and k != len(wordlist) - 2:
            ...
            elif k == len(wordlist) - 1 or k == len(wordlist) - 2:
                for s in wordlist[-3:]:
                    if s not in first_and_text:
                        first_and_text.append(s)
            else:
...
def sentence_split_ta_full(text, answer):
    wordlist = re.split(r'([。；？！：])', text)
    wordlist = ["".join(i) for i in zip(wordlist[0::2], wordlist[1::2])]
    lenth = len(wordlist)
    if len(wordlist) <= 5:
...
class LRDataset(torch.utils.data.Dataset):
    ...
    def __init__(self, file, batch_size, tokenizer, max_len, short_sampling_prob=0.1, sent_reverse_order=False,
        ...
        with Pool(threads) as p:
            ...
            data_list.extend(
                tqdm(
                    p.imap(annotate_, data, chunksize=32),
                    total=len(data),
    ...
    def __init__(self, file, batch_size, tokenizer, max_len, short_sampling_prob=0.1, sent_reverse_order=False,
        ...
        for i in data_list:
            for j in i:
                ...
                self.ex_list.append(sentence_split_full(text, question, answer))
        print('Load {0} documents'.format(len(self.ex_list)))
